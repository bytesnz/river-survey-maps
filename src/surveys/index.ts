...
export default function Surveys(element: HTMLElement, timescale: Timescale) {
  // Add sections
   ...
   */
  let createSurveyButtons = (parentElement: HTMLElement) => {
   ...
   */
  let addSurveyData = (type: string, data: Survey[]) => {
   ...
   * @returns {undefined}
   */
  let toggleSurvey = (survey?: string) => {
    let index;
   ...
   * @returns {undefined}
   */
  let toggleSurveyPart = (survey: string, part?: string) => {
    let index;
   ...
   * @returns {undefined}
   */
  let redrawSurveyData = (survey: string) => {
    ...
    if (typeof surveyData[survey] !== 'undefined') {
      ...
      Object.keys(surveyData[survey]).forEach((point) => {
        ...
        if (color) {
          // Create point
          let marker = L.circleMarker(latlng, Object.assign({}, config.defaultMarkerOptions, {
